...
enemy_cars = []  # Now stores dicts: {'rect': pygame.Rect, 'speed': int, 'lane': int}
    ...
    if random.randint(1, 40) == 1:
        ...
        if not lane_occupied:
            ...
            if not any(enemy_rect.colliderect(c['rect']) for c in enemy_cars):
                enemy_cars.append({'rect': enemy_rect, 'speed': speed, 'lane': lane})
...
def main():
    ...
    while running:
        ...
        draw_car(player_car, (0, 255, 0))
        for car in enemy_cars:
            draw_car(car['rect'], (255, 0, 0))
